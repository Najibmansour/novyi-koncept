"use client";
import Section from "../page 3/Section";
import Vector from "../svgs/logo/vectors/Vector";
import Vector1 from "../svgs/logo/vectors/Vector1";
import Vector2 from "../svgs/logo/vectors/Vector2";
import Vector3 from "../svgs/logo/vectors/Vector3";
import Vector4 from "../svgs/logo/vectors/Vector4";
import Vector5 from "../svgs/logo/vectors/Vector5";
import Vector6 from "../svgs/logo/vectors/Vector6";
import Vector7 from "../svgs/logo/vectors/Vector7";
import Vector8 from "../svgs/logo/vectors/Vector8";
import Vector9 from "../svgs/logo/vectors/Vector9";
import Vector10 from "../svgs/logo/vectors/Vector10";
import { useGSAP } from "@gsap/react";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import gsap from "gsap";

const vecs = [
  {
    service_id: "service_1",
    title: "Подбор товaрa для выходa нa мaркетплейcы",
    points: [
      `Выберем товар с помощью детальной аналитик – с расчетом затрат, цен, суммы на продвижение и конкуренции в данной нише`,
      "Дадим рекомендации по количеству единиц товара для первой поставки",
    ],
    dir: "right",
    vec: Vector,
  },
  {
    service_id: "service_2",
    title: "Рacчет юнит-экономики",
    points: [
      "Определим расходную часть при выводе товаров на маркетплейс",
      "Произведем все расчеты, связанные с закупкой товара, логистикой, расходами на комиссию маркетплейсов",
      `На основе этого дадим рекомендации как рассчитать действительно выгодную цену за 1 единицу товара.<br/> А также определим, какой бюджет надо заложить в продвижение, чтобы товар вышел в ТОП`,
    ],
    dir: "left",
    vec: Vector1,
  },

  {
    service_id: "service_3",
    title: "Запуск магазина с нуля",
    points: [
      "Поможем с выбором товара и регистрацией магазина на подходящем маркетплейсе: Wildberries, Ozon или Яндекс.Маркет",
      "Создадим дизайн карточек",
      "Загрузим товары и документы на маркетплейс. Заполним характеристики и другие обязательные поля",
      "Создадим медиа-контент (фото, видео, дизайн карточек, инфографика). Выделим преимущества товаров на фоне конкурентов",
      "Оформим первые поставки",
      "Проконсультируем и поддержим на всех этапах",
    ],
    dir: "right",
    vec: Vector10,
  },

  {
    service_id: "service_4",
    title: "Cоздaние кaрточки товaрa",
    points: [
      "Наша команда создаст вам продающие карточки товаров:  тексты, изображения и видео с учетом их дальнейшего SEO - продвижения",
      "Уникальный и индивидуальный дизайн каждой карточки поможет раскрыть характеристики товара, будет подчеркивать его преимущества перед конкурентами и соответствовать вашему бренду ",
      "Сделаем полное и качественное описание товара, подготовим SEO – оптимизированный текст",
      "Заполним правильно характеристики товара для попадания в популярные результаты поиска",
    ],
    dir: "left",
    vec: Vector3,
  },
  {
    service_id: "service_5",
    title: "Оформление поcтaвок",
    points: [
      "Управляем процессом оформления поставок, включая обработку заказов и контроль отправки товаров",
      "Соблюдаем требования маркетплейсов по  доставке и упаковке. Это помогает избежать возможных проблем и негативных последствий в виде штрафов",
      "Разберем важные моменты по моделям работы FBO / FBS ",
      "Вместе с вами создадим первую поставку и отправим товар на те склады, которые необходимы",
    ],
    dir: "right",
    vec: Vector4,
  },
  {
    service_id: "service_6",
    title: "SEO-продвижение и оптимизaция кaрточек товaров",
    points: [
      "Для начинающих селлеров поможем оптимизировать данные товаров с самого начала, что повысит вероятность привлечения клиентов и увеличит объем продаж",
      "Для действующих селлеров поможем выявить проблемные моменты и улучшить уже существующие карточки, что существенно увеличит эффективность продаж",
      "Проведем анализ конкурентов",
      "Составим семантическое ядро для ваших карточек. Пропишем ключевые запросы в заголовках и описании",
      "Составим продающие тексты для повышения конверсии ",
      "Создадим продающую карточку товара: дизайн, инфографика, фото и видео, заголовок, ключевые слова, описание, характеристики",
    ],
    dir: "left",
    vec: Vector5,
  },
  {
    service_id: "service_7",
    title: "Аудит карточек товаров в уже существующем магазине",
    points: [
      "Проверим все карточки магазина, а также выдачу карточек по регионам",
      ,
      "Оценим релевантность наименования товара, то есть соответствие поисковой выдаче запросу пользователя Выявим ошибки в продвижении товара",
      ,
      "Выявим ошибки в продвижении товара",
      ,
      "Проверим настройку рекламных кампаний и проанализируем участие в акциях",
      ,
      "Выявим все существующие точки роста магазина",
      ,
      "Составим готовый чек-лист по улучшению магазина со всеми рекомендациями и пояснениями",
    ],
    dir: "right",
    vec: Vector6,
  },
  {
    service_id: "service_8",
    title: " Настройка рекламных кампаний",
    points: [
      ,
      "Подберем необходимые рекламные кампании в соответствии с нишей и товарами в магазине",
      ,
      "Проведем комплексную аналитику инструментов продвижения (CTR, ДРР, продажи, конверсия)",
      ,
      "Скорректируем стратегии продвижения на основании аналитики",
      ,
      "Обеспечим эффективное участие в акциях маркетплейсов",
      ,
      "Регулярно будем анализировать и корректировать рекламные кампании",
      ,
      "Проведем ABC-анализ ассортимента и выявим товары повышенного спроса",
    ],
    dir: "left",
    vec: Vector7,
  },
  {
    service_id: "service_9",
    title: "Рaботa c отзывaми",
    desc: "Разработаем шаблоны правильных ответов на отзывы и обеспечим их своевременную публикацию. А так же поможем вам с ответами на вопросы",
    title2: "Почему это важно",
    points: [
      "Проведем анализ магазина для формирования отзывов, соответствующих требованиям маркетплейса",
      "Рассчитаем оптимальное количество отзывов, необходимых для продвижения вашего товара ",
      "Напишем реалистичные отзывы (по определенному алгоритму, без шаблонов)",
      "Сформулируем ответы на положительные и отрицательные отзывы ((по определенному алгоритму, без шаблонов)",
    ],
    dir: "right",
    vec: Vector8,
  },
  {
    service_id: "service_10",
    title: "Полное сопровождение магазина",
    points: [
      "Полностью освободим вас от операционной работы в вашем магазине на маркетплейсе, а также увеличим продажи и выручку магазин",
      "Услуга подойдет начинающим и опытным предпринимателям, у которых уже есть магазин на маркетплейсе и по нему было хотя бы одна продажа, при этом сейчас стоит задача увеличить продажи и делегировать ведение магазина",
    ],
    dir: "left",
    vec: Vector9,
  },
];

export default function Page3() {
  gsap.registerPlugin(ScrollTrigger);
  useGSAP(() => {
    let boxes = gsap.utils.toArray(".Box");
    // console.log(boxes);
    boxes.forEach((box) => {
      let mm = gsap.matchMedia();
      let breakPoint = 800;

      mm.add(
        {
          // set up any number of arbitrarily-named conditions. The function below will be called when ANY of them match.
          isDesktop: `(min-width: ${breakPoint}px)`,
          isMobile: `(max-width: ${breakPoint - 1}px)`,
          reduceMotion: "(prefers-reduced-motion: reduce)",
        },
        (cont) => {
          let { isDesktop, isMobile } = cont.conditions;
          gsap.fromTo(
            box,
            {
              x: isDesktop ? (box.id == "left" ? "100%" : "-100%") : 0,
              y: isMobile ? "20%" : 0,
              opacity: 0,
              scrollTrigger: {
                trigger: box,
                // start: "top bottom-=150",
                scrub: isDesktop ? true : false,
                // end: "bottom bottom-=150",
                markers: false,
              },
            },
            {
              x: 0,
              y: 0,
              opacity: 1,
              scrollTrigger: {
                trigger: box,
                start: isDesktop ? "top100 bottom-=150" : "top center-=100",
                scrub: isDesktop ? true : false,
                end: isDesktop ? "bottom bottom-=150" : "bottom bottom-=150",
                markers: false,
              },
            },
          );
        },
      );
    });
  });

  return (
    <section id="Нашиуслуги" className="flex flex-col items-center lg:gap-10 ">
      <h2
        id="Нашиуслуги"
        className="fontLogo text-2xl font-bold text-white lg:text-5xl"
      >
        Наши услуги:{" "}
      </h2>
      <div className="flex w-screen flex-row justify-center lg:mb-10">
        <div className="w-[85%] lg:w-[76vw]">
          <ul className="flex flex-col">
            {vecs.map((vec, i) => (
              <Section direction={vec.dir} Img={vec.vec} key={i} id={vec.dir}>
                <h3 className="mb-3 text-center text-xl font-bold tracking-wider lg:text-left lg:text-3xl">
                  {vec.title}
                </h3>
                <ul className="list-outside list-disc marker:bg-white lg:text-lg ">
                  {vec.points.map((point, i) => (
                    <li key={i} className="tracking-tighter">
                      {<p dangerouslySetInnerHTML={{ __html: point }}></p>}
                    </li>
                  ))}
                </ul>
              </Section>
            ))}
          </ul>
        </div>
      </div>
    </section>
  );
}
